...
### A. 理想位置ベースの精密走査
ROIをリサイズ（高さ16px固定）した画像に対し、各文字の理想的な出現位置（102px基準）を中心に12pxの範囲をスキャンする。
-   **探索範囲**: 理想的な左端座標から -6px 〜 +6px の範囲。
...
バッファ内の最大スコア群に対し、以下の条件を全て満たした場合にWIPEOUTと確定させる。
1.  **高精度一致**: 「80%以上が2文字以上」
2.  **複数文字検知**: 「75%以上が3文字以上」
3.  **最低品質保証**: 「全7文字が60%以上」
